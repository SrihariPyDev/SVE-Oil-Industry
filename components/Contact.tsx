...
export default function Contact() {
  const [form, setForm] = useState<FormState>({
    name: "", phone: "", email: "", company: "", industry: "", product: "", message: "",
  });
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name === "industry") {
      setForm((prev) => ({ ...prev, industry: value, product: "" }));
    } else {
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <div className="text-white/80 text-xs mt-0.5">{COMPANY.contact.phoneDisplay}</div>
              </div>
              <svg className="w-4 h-4 text-white ml-auto shrink-0 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <a href={COMPANY.googleMapsLink} target="_blank" rel="noopener noreferrer"
                    className="text-gray-600 text-sm hover:text-[#0f172a] transition-colors leading-snug break-words">
          ...
          <div className="lg:col-span-3">
            <div className="bg-white rounded-3xl p-6 sm:p-8 lg:p-10 shadow-lg border border-gray-100">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {/* ── Two-Step: Step 2 — Product (shown after industry selected) ── */}
                {form.industry && (
                  <div className="animate-fade-in-up">
                    <label htmlFor="contact-product" className="block text-[11px] font-bold text-gray-500 mb-1.5 uppercase tracking-wide">
                    ...
                    {filteredProducts.length > 0 && (
                      <p className="mt-1.5 text-[11px] text-gray-400">
                        {filteredProducts.length} product{filteredProducts.length !== 1 ? "s" : ""} available for this industry.
                      </p>
                    )}
                  </div>
                )}
